...
sudo systemctl daemon-reload #only if systemd files were changed
...
1) vault credentials - take romantest pipeline
2) download smth from nexus (example our agents)
3) check sonar: https://ci.svc.ifortuna.cz/job/Openshift/job/testomat-sonarqube/job/testomat/job/master/183/replay/ -> take this pipeline
4) ask some developoer to check jenkins test blue ocean
