...
contains files for the bootloader
            ...
            /boot — simple explanation
            /boot is a directory in Linux that contains the files needed to start the operating system.
            Think of it as the startup folder of the whole computer.
            When the computer turns on, it must load some files before Linux can even run.
            Those files are stored in /boot.

            What is a bootloader
            A bootloader is a small program that starts the operating system.
            In most Linux systems it is:
...
# /proc — Virtual Filesystem (procfs)

## Simple Explanation

`/proc` is a special directory in Linux that shows **information about the system and running processes**.

However, it is **not a real directory stored on disk**.

It is a **virtual filesystem** created by the Linux kernel.

---

# What "Virtual Filesystem" Means

Normally files exist physically on disk.
...
This file actually exists on the disk.

But `/proc` is different.

Files inside `/proc` are **generated by the kernel in real time**.

They exist only **in memory**, not on disk.

---

# Why `/proc` Exists

Linux needs a simple way to expose **system information**.

Instead of creating special commands for everything, Linux presents this information as **files**.

Programs and administrators can read these files just like normal files.
...
# Examples of Information in `/proc`
...
This shows system memory statistics.

---

# Process Information

`/proc` also contains information about **running processes**.
...
Find the process ID of your shell:
...
Now check the process directory:

```
ls /proc/3456
```

You will see files describing that running process.
...
Kernel (system information)
      ...
      /proc
        ↓
 files representing system data
```

So you can think of `/proc` as:
...
Many system monitoring tools read information from `/proc`.
...
These tools simply read data from `/proc`.
...
- are created dynamically by the kernel
- constantly change
- disappear when processes stop
...
`/proc` is a **virtual filesystem created by the Linux kernel that exposes system and process information as files.**
